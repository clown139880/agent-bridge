...
export const TOOL_SPECS: readonly ToolSpec[] = [
  ...
  { name: 'agent_bridge_submit_turn', description: 'Continue an external session using server-resolved auto/steer/start semantics.', parameters: { sessionId: id('External session id'), input: id('Non-empty turn input'), delivery: { type: 'string', enum: ['auto', 'steer', 'start_turn'] }, expectedTurnId: optionalString('Current turn id for concurrency protection'), model: optionalString('Optional configured Codex model; only for a new turn') }, domain: 'bridge', operation: 'submit_turn', mutates: true },
