...
export const TOOL_SPECS: readonly ToolSpec[] = [
  ...
  { name: 'agent_bridge_create_session', description: 'Create a session on a real listed worker, optionally starting its first turn.', parameters: { workerId: id('Worker id returned by list workers'), workspace: id('Absolute workspace returned by that worker'), input: optionalString('Optional first-turn input') }, domain: 'bridge', operation: 'create_session', mutates: true },
  { name: 'agent_bridge_submit_turn', description: 'Continue an external session using server-resolved auto/steer/start semantics.', parameters: { sessionId: id('External session id'), input: id('Non-empty turn input'), delivery: { type: 'string', enum: ['auto', 'steer', 'start_turn'] }, expectedTurnId: optionalString('Current turn id for concurrency protection') }, domain: 'bridge', operation: 'submit_turn', mutates: true },
