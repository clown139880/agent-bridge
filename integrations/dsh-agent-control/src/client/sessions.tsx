...
export function Sessions({ store, views: providedViews, refreshToken, initialSessionId, surface = 'both', onRefresh, onBack }: { store: SessionStore; views?: SessionViewStore; refreshToken: unknown; initialSessionId?: string | undefined; surface?: 'both' | 'list' | 'detail'; onRefresh?: () => void; onBack?: () => void }) {
  ...
  const composer = selectedId && detail ? <>
    ...
    <SessionComposer value={detail.draft} busy={Boolean(busy)} canSend={Boolean(canAct)} active={selected?.['status'] === 'active'} attachments={detail.draftAttachments}
      ...
      onAddImage={image => void store.addDraftImage(selectedId, image)} onRemoveAttachment={id => store.removeDraftAttachment(selectedId, id)} />
  </> : null
...
function ApprovalCard({ item, disabled, decide }: { item: JsonObject; disabled: boolean; decide(choice: string): void | Promise<void> }) {
  ...
  // Prefer DSH's native approval panel for a consistent look; fall back to our own buttons
  // if it isn't exported by this client build. Both call the same decide() → Bridge API.
