...
describe('Cordis lifecycle', () => {
  it('registers tools, prompt, route and disposes the shared service', async () => {
    ...
    const fetchRegister = vi.fn(() => routeDispose)
    ...
    // The channel registration injects both `connection` and `webServer`:
    // we mount the prefix route with `connection.register(scoped, …)`, passing our
    // own `webServer`-injected fiber as the owner, so the inject callback only
    // fires once both are present.
    ctx.provide('connection', { register, fetch: { register: fetchRegister } } as never)
    ctx.provide('webServer', { register: vi.fn(() => routeDispose) } as never)
    ...
    expect(register).toHaveBeenCalledWith(expect.anything(), '/agent-control', expect.any(Function))
