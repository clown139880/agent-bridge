...
test("a 0.4.2 database upgrades transactionally to the Agent Control schema", () => {
  ...
  for(const table of ["pending_requests","actions","idempotency_keys","stream_events","deleted_sessions","attachments"])assert.ok(tables.includes(table));
  ...
  store.db.close();const reopened=new Store(path);assert.equal((reopened.db.prepare("SELECT COUNT(*) AS n FROM schema_migrations").get() as any).n,6);
