...
export class SessionActionService {
  ...
  existing(principal:string,key:string,path:string,body:unknown):ActionRow|undefined{
    try{return this.store.findIdempotent(principal,key,"POST",path,body);}
  ...
  create(input:{principal:string;key:string;path:string;body:unknown;kind:ActionKind;machineId:string;sessionId?:string}):
    {action:ActionRow;existing:boolean}{
    try{return this.store.createAction({principal:input.principal,key:input.key,method:"POST",path:input.path,
